...
class chessgames(unittest.TestCase):
    def test(self):
        w, d, ed, m = (
            rf.winner(a_pgn_game),
            rf.date_of_game(a_pgn_game),
            rf.diff_elo(a_pgn_game),
            rf.moves(a_pgn_game),
        )
        self.assertEqual(w, "ΙΣΟΠΑΛΙΑ")
        self.assertEqual(d, "09-04-2023")
        self.assertEqual(ed, 7)
        self.assertEqual(m, 49)
